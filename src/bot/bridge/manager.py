...
class ChannelBridgeManager:
    ...
    async def handle_message(self, message: discord.Message) -> None:
        ...
        for route in routes:
            ...
            if destination is None:
                print(f'Bridge destination not found for guild={route.dst.guild} channel={route.dst.channel}')
                continue
            ...
            payload = await self._build_mirror_payload(source_message=message, profile=profile, dicebear_failed=dicebear_failed, target=route.dst)
            if payload is None:
                continue
    ...
    async def _resolve_channel(self, endpoint: ChannelEndpoint) -> Optional[discord.abc.Messageable]:
        ...
        try:
            ...
            print(f'Failed to fetch destination channel guild={endpoint.guild} channel={endpoint.channel}: {exc}')
            return None
        return fetched
    ...
    async def _build_mirror_payload(self, *, source_message: discord.Message, profile: BridgeProfile, dicebear_failed: bool, target: ChannelEndpoint) -> Optional[MirrorPayload]:
